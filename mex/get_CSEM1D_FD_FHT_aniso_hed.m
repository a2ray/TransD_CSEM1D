%#codegen
function [Er,Eb,Hr,Hb,Ez,Hz] = get_CSEM1D_FD_FHT_aniso_hed(freqs,rRx,zRx,zTx,z,sig,filterName,...
                                       lUseLaggedConv)
%
...
%for codegen 
    ...
    if (lUseLaggedConv==1)
        ...
        EHK   = 1i*zeros(10*length(freqs),length(lambdaR));
    else
        EHK   = 1i*zeros(10,length(Filter.base));
    end
        ...
        if (lUseLaggedConv==1)
            ...
            for iFreq = 1:nF 
                ...
                for i = 1:length(lambda)   
                    ...
                    EHK((2*iFreq-1):2*iFreq,i)=ErK;
                    EHK(2*nF+(2*iFreq-1):2*nF+2*iFreq,i)=EbK;
                    EHK(4*nF+(2*iFreq-1):4*nF+2*iFreq,i)=HrK;
                    EHK(6*nF+(2*iFreq-1):6*nF+2*iFreq,i)=HbK;
                    EHK(8*nF+iFreq,i)=EzK;
                    EHK(9*nF+iFreq,i)=HzK;
                end
            ...
            for irR = 1:length(rR);
                ...
                eyBase(irR,4*nF+1:6*nF,:) = (EHK(8*nF+1:10*nF,iShift)*Filter.J1)/rR(irR);
                eyBase(irR,1:4*nF,:)= ([EHK(1:2:8*nF-1,iShift),EHK(2:2:8*nF,iShift,:)]*[Filter.J0;Filter.J1/rR(irR)])/rR(irR);
            end
            ...
            % PPV = ppval(PP,log10(rRx));
             ...
             Ez = EH(4*nF+1:5*nF,:)';
             Hz = EH(5*nF+1:6*nF,:)';
        else % Normal call without the lagged convolution + spline approximations:
             for iFreq = 1:nF 
                f = freqs(iFreq);
                %
                % Loop over receivers
                ...
                for iRx = 1:length(rRx)
                    ...
                    for i = 1:length(lambda)   
                        %all components of the EH field
                         [ErK,EbK,HrK,HbK,EzK,HzK] = getCSEM1DKernels_aniso_hed(lambda(i),f,zRxEval,zTx,z,sig);
                         EHK(1:10,i)=[ErK;EbK;HrK;HbK;EzK;HzK];
                    end    
                    ...
                    %
                    Er(iRx,iFreq) = ( EHK(1,:)*Filter.J0 + EHK(2,:)*Filter.J1 )/rRxEval; 
                    Eb(iRx,iFreq) = ( EHK(3,:)*Filter.J0 + EHK(4,:)*Filter.J1 )/rRxEval;
                    Hr(iRx,iFreq) = ( EHK(5,:)*Filter.J0 + EHK(6,:)*Filter.J1 )/rRxEval; 
                    Hb(iRx,iFreq) = ( EHK(7,:)*Filter.J0 + EHK(8,:)*Filter.J1 )/rRxEval;
                    Ez(iRx,iFreq) = ( EHK(9,:)*Filter.J1 )/rRxEval; 
                    Hz(iRx,iFreq) = ( EHK(10,:)*Filter.J1 )/rRxEval;
                    %
...
% TM mode, pzII
    ...
    [curlyRA,curlyRD] = get_curly_R(Rs_u,Rs_d,pzII(iTxLayer),zRx,z,iTxLayer,omega);
...
% Kernels according to Loseth, without the bessel functions multiplied
    ...
    %Ez
    EzKernels         = krho^2*gD_TM*1i/(4*pi*omega*epsc(2,iTxLayer));
...
function [finRA,finRD] = get_curly_R(Rs_u,Rs_d,pz,zR,z,iTxLayer,omega)
    %global omega
